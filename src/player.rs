...
fn init(mut commands: Commands, mut players: Query<(Entity, &mut Player), Added<Player>>) {
...
fn controller(
    ...
    player: Single<(&Player, &mut Physics), With<Player>>,
    mut transforms: Query<&mut Transform>,
    keys: Res<ButtonInput<KeyCode>>,
    mut mouse: EventReader<MouseMotion>,
) {
    let sensivity = 0.12;
    let (player, mut physics) = player.into_inner();

    let mut transform_camera = transforms.get_mut(player.camera).unwrap();
